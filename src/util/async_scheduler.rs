use super::redis::transaction;
use futures_util::FutureExt;
use redis::{
  aio::Connection, AsyncCommands, FromRedisValue, 
...
impl<T: Callable<A> + DeserializeOwned + Serialize, A> Scheduler<T, A> {
  ...
  pub async fn clear_ready_jobs<'a>(&'a mut self, timestamp: i64) -> RedisResult<()> {
    ...
    let _ = transaction(&mut self.connection, &[jobs_key, schedule_key], |con, pipe| {
      (async move {
        let ready_jobs = con.zrangebyscore::<&str, &str, i64, Vec<String>>(
