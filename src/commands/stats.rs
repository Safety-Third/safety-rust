...
pub fn stats_commands(commands: &mut CreateApplicationCommands) -> &mut CreateApplicationCommands {
  fn guild_id(sub: &mut CreateApplicationCommandOption) -> &mut CreateApplicationCommandOption {
    sub
  ...
  commands.create_application_command(|command| {
    command
      ...
      .create_option(|consent| {
        consent
          .name("consent")
          .kind(ApplicationCommandOptionType::SubCommandGroup)
          .description("Manage consent for collecting reaction stats")
          .create_sub_option(|approve| {
            approve
              .name("approve")
              .kind(ApplicationCommandOptionType::SubCommand)
              .description("Approve of collecting stats of reactions in a server")
              .create_sub_option(guild_id)
          })
          .create_sub_option(|delete| {
            delete
              .name("delete")
              .kind(ApplicationCommandOptionType::SubCommand)
              .description(
                "Revoke permission and immediately delete all collected reaction stats in a server",
              )
              .create_sub_option(guild_id)
          })
          .create_sub_option(|delete| {
      ...
      .create_option(|get| {
        get
          ...
          .kind(ApplicationCommandOptionType::SubCommand)
          .description(
            "See how often you use one or more emojis in a server. These stats are direct messaged",
          )
          .create_sub_option(guild_id)
          .create_sub_option(|num_emojis| {
            num_emojis
...
pub async fn interaction_stats_entrypoint(
  ctx: &Context,
...
async fn intereaction_stats_consent(
  ctx: &Context,
  interaction: &ApplicationCommandInteraction,
) -> Result<(), String> {
  ...
  let (guild_name, redis_key) =
    get_name_and_key_from_interaction(ctx, interaction, id_or_name_key).await?;
  ...
  if let Err(why) = interaction
    .user
    .direct_message(ctx, |msg| msg.content(result))
    .await
  {
...
async fn intereaction_stats_usage(
  ctx: &Context,
  interaction: &ApplicationCommandInteraction,
) -> Result<(), String> {
  let subdata = &interaction.data.options[0].options;
  ...
  let (guild_name, redis_key) =
    get_name_and_key_from_interaction(ctx, interaction, id_or_name).await?;
  ...
  let result = if react_stats.len() <= 1 {
    ...
    if let Some(specific_emoji) = emoji {
      ...
      if react_count == &1 {
        ...
        format!(
          "You have used {} {} times in {}",
          specific_emoji, react_count, guild_name
        )
      }
    } else {
...
async fn get_name_and_key_from_interaction(
  ctx: &Context,
  interaction: &ApplicationCommandInteraction,
  id_or_name: Option<&str>,
) -> Result<(String, String), String> {
  match id_or_name {
    Some(identifier) => match get_guild_from_id(ctx, &identifier).await {
      Some(guild) => Ok((
        guild.name.clone(),
        format!("{}:{}", interaction.user.id.0, guild.id.0),
      )),
      None => Err(format!("Could not find a server {}", identifier)),
    },
    None => {
      if let Some(id) = interaction.guild_id {
        if let Some(name) = id.name(&ctx.cache) {
        ...
        Err(String::from(
          "You must provide a guild id/name. Alternatively, you can message in a server )channel",
        ))
      }
    }
  }
...
async fn get_guild_from_id(ctx: &Context, id_or_name: &str) -> Option<Guild> {
  match id_or_name.parse::<u64>() {
    Ok(id) => ctx.cache.guild(id),
    Err(_) => {
      let guilds = &ctx.cache.guilds();

      for guild in guilds.iter() {
        if let Some(name) = guild.name(&ctx.cache) {
          if name == id_or_name {
            return guild.to_guild_cached(&ctx.cache);
          }
        }
      }
