...
pub async fn briefing(ctx: &Context, original_msg: &Message, args: Args) -> CommandResult {
  let mut text: Vec<&str> = args.message()
  .split("\n")
  .collect();
  let heading = text[0];
  ...
  if let Err(_) = Guild::get(&ctx, guild_id)
    ...
    original_msg.reply(ctx, "You are not approved")
      .await?;
    return Ok(())
  }

  let preview = format!(concat!(
  ...
  if accepted {
    ...
    let serialized = match bincode::serialize(&briefing) {
      Ok(data) => data,
      Err(_) => {
        msg.reply(ctx, "Could not serialize your briefing. Please but bot manager")
          .await?;
        return Ok(())
      }
    };
    ...
    let result = {
      ...
      redis_client.0.zadd::<&str, i64, Vec<u8>, u64>(BRIEFING_KEY, serialized, now.timestamp()).await
    };

    match result {
      Ok(_) => msg.reply(ctx, "Your briefing was accepted").await?,
      Err(error) => msg.reply(ctx, format!("Could not save your briefing: {:?}", error)).await?
    };
  } else {
...
pub async fn send_briefing(briefings: &MyVec, channel_id: u64, http: &Arc<Http>) -> Result<(), String> {
  let mut messages: Vec<String> = vec![];
  let mut current_message = String::from("*DOOTDOOTDOOTDOOT*. It's time for your Safety Weekly Briefing!\n\n");
