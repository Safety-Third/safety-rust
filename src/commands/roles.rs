...
where
  F: FnMut(&mut Vec<RoleId>, &Vec<RoleId>),
{
  ...
  for role in args.iter::<String>() {
    match role {
      Ok(role_string) => match get_role_from_string(&guild, &role_string) {
  ...
  match guild
    .edit_member(&ctx, member_id, |m| m.roles(&roles))
    .await
  {
    Ok(_) => {
      ...
      let _ = msg
        ...
        .say(
          ctx,
          format!("New roles for {}: {:?}", member.mention(), roles),
        )
        .await;
